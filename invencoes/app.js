...
let playerOne='',playerTwo='',playerName='',teamDisplay='';
...
function cleanPerson(value){return value.trim().replace(/\s+/g,' ').slice(0,10)}
...
function makeTeamName(a,b){return [a,b].sort((x,y)=>x.localeCompare(y,'pt-BR',{sensitivity:'base'})).join(' + ')}
...
function levelText(){return index<17?'Observe':index<34?'Pensem na transformação':'Desafio de inventores'}

function roleText(){
  const reader=index%2===0?playerOne:playerTwo;
  const helper=index%2===0?playerTwo:playerOne;
  return `📖 ${reader} lê. 🤝 ${helper} observa e ajuda. Depois, decidam juntos.`;
}

function render(){
 const item=session[index];stage=1;
 document.getElementById('roundText').textContent=`Item ${index+1} de ${TOTAL}`;
 document.getElementById('playerLabel').textContent=`👥 ${playerOne} + ${playerTwo}`;
...
function classify(choice){
 ...
 document.getElementById('feedbackTitle').textContent=ok?'✅ Vocês acertaram!':'Ihhhh 😢';
...
function showFollow(){
 ...
 document.getElementById('followQuestion').textContent=`Conversem de novo: ${item.q}`;const box=document.getElementById('options');box.innerHTML='';
...
function answerFollow(opt){
 ...
 tone(ok);document.getElementById('scoreText').textContent=score;
 const f=document.getElementById('feedback');f.className='feedback '+(ok?'good':'bad');f.classList.remove('hidden');
 document.getElementById('feedbackTitle').textContent=ok?'⭐ Boa, dupla!':'Ihhhh 😢';
 document.getElementById('feedbackText').textContent=ok?item.ans:`A resposta é: ${item.ans}.`;
 document.getElementById('nextBtn').textContent=index===TOTAL-1?'Ver a pontuação da dupla 🏆':'Próximo item →';document.getElementById('nextBtn').onclick=next;
}

function next(){index++;if(index>=TOTAL)return finish();render()}

function medalFor(points){
 if(points>950)return{key:'gold',icon:'🥇',title:'Medalha de ouro!',message:`Parabéns, ${teamDisplay}! Vocês ganharam a medalha de ouro. Chamem o professor!`};
 if(points>800)return{key:'silver',icon:'🥈',title:'Medalha de prata!',message:`Parabéns, ${teamDisplay}! Vocês ganharam a medalha de prata. Chamem o professor!`};
 if(points>600)return{key:'bronze',icon:'🥉',title:'Medalha de bronze!',message:`${teamDisplay}, vocês ganharam a medalha de bronze. Joguem de novo juntos e tentem chegar à prata!`};
 return{key:'none',icon:'🎯',title:'Tentem mais uma vez!',message:`${teamDisplay}, conversem bastante e joguem de novo para tentar conquistar uma medalha!`};
}
...
function rankingMedal(points){
 if(points>950)return'🥇';
...
function drawRanking(box,ranking,highlight='',offline=false){
 box.innerHTML='';
 if(!ranking.length){
  const empty=document.createElement('div');empty.className='rank-empty';empty.textContent=offline?'Sem internet. Ainda não há pontuações neste computador.':'Ainda não há pontuações no ranking da escola.';box.appendChild(empty);return;
 }
 ranking.forEach((entry,i)=>{
  ...
  const n=document.createElement('span');n.className='rank-name';n.textContent=`${rankingMedal(Number(entry.score))} ${entry.name}`;
 ...
 if(offline){const note=document.createElement('div');note.className='rank-empty';note.textContent='Sem conexão: mostrando apenas os resultados deste computador.';box.appendChild(note)}
}

async function renderRanking(targetId,highlight=''){
 const box=document.getElementById(targetId);if(!box)return;
 box.innerHTML='<div class="rank-empty">Carregando ranking da escola...</div>';
 try{
...
async function finish(){
 ...
 const box=document.getElementById('summaryRanking');box.innerHTML='<div class="rank-empty">Salvando a pontuação da dupla...</div>';
...
function start(){
 ...
 const one=cleanPerson(inputOne.value||playerOne);
 const two=cleanPerson(inputTwo.value||playerTwo);
 if(!one||!two){document.getElementById('nameError').classList.remove('hidden');(!one?inputOne:inputTwo).focus();return}
 playerOne=one;playerTwo=two;
 inputOne.value=playerOne;inputTwo.value=playerTwo;
 playerName=makeTeamName(playerOne,playerTwo);
 teamDisplay=`${playerOne} e ${playerTwo}`;
...
function goHome(){
 ...
 renderRanking('homeRanking',playerName);show('home');
}
...
document.getElementById('playerOne').addEventListener('keydown',e=>{if(e.key==='Enter')document.getElementById('playerTwo').focus()});
